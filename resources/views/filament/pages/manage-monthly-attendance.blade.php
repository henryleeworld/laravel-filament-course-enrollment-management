                ...
                <div class="fi-section-content p-6">
                    <div class="fi-ta-empty-state-content mx-auto flex max-w-lg flex-col items-center justify-center text-center">
                        ...
                        <p class="fi-ta-empty-state-description text-sm text-gray-500 dark:text-gray-400">
                            {{ __('There are no students enrolled in this course for the selected month.') }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="fi-section rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10">
                <div class="fi-section-content p-6">
                    <div class="fi-ta-empty-state-content mx-auto flex max-w-lg flex-col items-center justify-center text-center">
                        ...
                        <h4 class="fi-ta-empty-state-heading text-base font-semibold leading-6 text-gray-950 dark:text-white">
                            {{ __('No course classes scheduled') }}
                        </h4>
                        <p class="fi-ta-empty-state-description text-sm text-gray-500 dark:text-gray-400">
                            {{ __('There are no course classes scheduled for this month.') }}
                        </p>
                    </div>
                </div>
            </div>
                                    ...
                                    <thead class="divide-y divide-gray-200 dark:divide-white/5">
                                        <tr class="bg-gray-50 dark:bg-white/5">
                                            ...
                                            @foreach($courseClasses as $courseClass)
                                                @php
                                                $scheduledDate = $courseClass->scheduled_date;
                                                $scheduledDate->locale(config('app.locale'));
                                                $scheduledDate->settings(['formatFunction' => 'translatedFormat']);
                                                @endphp
                                                    ...
                                                    <span class="group flex w-full items-center justify-center">
                                                        <span class="fi-ta-header-cell-label text-sm font-semibold text-gray-950 dark:text-white">
                                                            ...
                                                            <div class="text-xs text-gray-500 dark:text-gray-400">
                                                                {{ \Illuminate\Support\Carbon::parse($courseClass->start_time)->format('H:i') }} - {{ \Illuminate\Support\Carbon::parse($courseClass->end_time)->format('H:i') }}
                                                            </div>
                                                        </span>
                                                    </span>
                                                </th>
